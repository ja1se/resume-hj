      ...
      <div className="w-full flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-14">
        ...
        >
          ...
          <div className="flex flex-col gap-2 lg:gap-3 relative">
            <span
              className={cn(
                "bg-number absolute -top-8 lg:-top-10 text-[80px] lg:text-[120px] font-display text-slate-900/5 dark:text-slate-100/5 leading-none select-none transition-colors duration-300",
                isRightAlign ? "-right-2 lg:-right-4" : "-left-2 lg:-left-4",
              )}
            >
              0{number}
            </span>
            <span className="animate-text text-violet-500 dark:text-violet-400 font-medium text-sm lg:text-[18px] tracking-[0.2em] uppercase">
              {subtitle}
            </span>
            <h2 className="animate-text text-4xl lg:text-[64px] font-medium text-slate-800 dark:text-slate-100 leading-[1.1] font-display transition-colors duration-300">
              {title}
            </h2>
          </div>

          {/* Details */}
          <div className="animate-text flex flex-col gap-4 lg:gap-5 text-slate-500 dark:text-slate-400 text-base lg:text-[18px] leading-relaxed max-w-[480px] transition-colors duration-300">
            {[
              { label: "Technical Stack", value: techStack },
              { label: "Output", value: `${duration} / ${contribution}` },
            ].map((item) => (
              <div key={item.label} className="flex flex-col gap-1">
                <span className="font-medium text-slate-800 dark:text-slate-200 text-xs lg:text-[14px] uppercase tracking-wider">
                  {item.label}
                </span>
        ...
        >
          ...
          <div className="w-full relative overflow-hidden rounded-[12px] lg:rounded-[20px] shadow-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 transition-colors duration-300">
            <ProjectImage
              src={image}
              alt={title}
